<?php
    ...
    class Display_slug extends CI_Controller
    {
        public function __construct()
        {
            parent::__construct();
            $this->load->model(array('display_slug_model', 'Search_model'));
            $this->load->helper('url_helper');
        }

        public function view($slug = NULL)
        {
            $data["store"] = $this->display_slug_model->create_slug($slug);            
        ...
        public function fetch_store_data($slug)
        {
            $result = $this->display_slug_model->get_store_data($slug);
        ...
        public function actual_revenue($slug)
        {
            $data = $this->display_slug_model->store_sales($slug);
            echo json_encode($data["store_revenue"]);
        }

        public function last_week_rate($slug)
        {
            $today_sales = $this->display_slug_model->today_sales($slug);
            $last_week_revenue = $this->display_slug_model->last_week_sales($slug);
        ...
        public function last_month_rate($slug)
        {
           
            $today_sales = $this->display_slug_model->today_sales($slug);
            $last_month_revenue = $this->display_slug_model->last_month_sales($slug);
        ...
        public function last_year_rate($slug)
        {
            $today_sales = $this->display_slug_model->today_sales($slug);
            $last_year_revenue = $this->display_slug_model->last_year_sales($slug);
